...
func offset_thumb_transform(new_transform: Transform3D, is_right: bool) -> Transform3D:
	...
	transform_offset = transform_offset.rotated(Vector3.RIGHT, PI).rotated(Vector3.FORWARD, PI)
	
	return transform_offset


func pose_thumb(bone: UltraleapBone, bone_id: int, is_right: bool) -> void:
	...
	bone_transform = reference_transform.affine_inverse()*bone_transform

	# set pose
	skeleton.set_bone_pose_rotation(bone_id, bone_transform.basis.get_rotation_quaternion())
	#skeleton.set_bone_global_pose_override(bone_id, bone_transform.basis.get_rotation_quaternion(), 1.0, true)


func pose_finger(bone: UltraleapBone, bone_id: int, is_right: bool) -> void:
...
func _process(_delta):
	...
	if frame != null and frame.is_right_hand_visible:
		...
		pose_thumb(frame.right_hand.thumb.intermediate, 39, true)
		pose_thumb(frame.right_hand.thumb.distal, 40, true)
	...
	if frame != null and frame.is_left_hand_visible:
		...
		#fingers
		pose_finger(frame.left_hand.thumb.proximal, 13, false)
		pose_thumb(frame.left_hand.thumb.intermediate, 14, false)
		pose_thumb(frame.left_hand.thumb.distal, 15, false)
