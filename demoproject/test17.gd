...
func _process(_delta: float) -> void:
	...
	if frame != null and frame.is_right_hand_visible:
		# arm
		pose_arm(frame.right_hand.arm, 33)
	...
	if frame != null and frame.is_left_hand_visible:
		# arm
		pose_arm(frame.left_hand.arm, 9)
...
func pose_thumb(bone: UltraleapBone, bone_id: int) -> void:
	...
	match bone_id:
		11:
			bone_transform = bone_transform.rotated_local(Vector3.UP, deg_to_rad(thumb_yaw)).rotated_local(Vector3.UP, deg_to_rad(-thumb_roll))
			pass
		12,13:
			bone_transform = bone_transform.rotated(Vector3.UP, deg_to_rad(thumb_yaw)).rotated(Vector3.UP, deg_to_rad(-thumb_roll))
			pass
		35:
			bone_transform = bone_transform.rotated_local(Vector3.UP, deg_to_rad(-thumb_yaw)).rotated_local(Vector3.UP, deg_to_rad(thumb_roll))
			pass
...
func pose_arm(bone: UltraleapBone, bone_id: int) -> void:
	...
	var rest_transform: Transform3D = skeleton.get_bone_global_rest(bone_id)
	var root_transform: Transform3D = skeleton.get_bone_global_rest(0)
	var check = rest_transform.origin
	...
	skeleton.set_bone_pose_rotation(bone_id, bone_transform.basis.get_rotation_quaternion())
	var bone_origin: Vector3 = bone_transform.origin
	bone_origin = rest_transform.origin - bone_transform.origin
	var bone_start = bone_origin
	
	bone_origin.z = -bone_start.z + 2*rest_transform.origin.y - leap_origin.y
	
	if bone_id == 33:
		bone_origin.y = -bone_start.y - 2*rest_transform.origin.x + leap_origin.x
		bone_origin.x = -bone_start.x + rest_transform.origin.z - leap_origin.z
	else:
		bone_origin.y = -bone_start.y + 2*rest_transform.origin.x - leap_origin.x
		bone_origin.x = -bone_start.x - rest_transform.origin.z + leap_origin.z
	
		bone_origin = bone_origin
	
	#skeleton.set_bone_pose_position(bone_id, bone_origin)
