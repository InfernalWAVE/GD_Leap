...
func pose_from_transfrom(new_transform: Transform3D, bone_id: int) -> void:
	...
	var rest_transform = Transform3D(skeleton.get_bone_global_rest(bone_id).basis)
	var local_rest_transform = skeleton.get_bone_rest(bone_id)
	
	# undo offset from rigging transform.
	var c = bone_transform.basis
	#bone_transform = Transform3D(Basis(Vector3(-c.z.x, c.z.y, -c.z.z),  Vector3(-c.y.x, c.y.y, -c.y.z), Vector3(c.x.x, -c.x.y, c.x.z)))
	var what_is_this = Transform3D(c).affine_inverse()*bone_transform
	
	# get pose relative to parent bone
	# TODO cache global transforms to avoid recalculations
	var parent_transform: Transform3D = skeleton.get_bone_global_pose(skeleton.get_bone_parent(bone_id)).basis
	var parent_local_transform: Transform3D = skeleton.get_bone_pose(skeleton.get_bone_parent(bone_id)).basis
	bone_transform = parent_transform.affine_inverse()*bone_transform
	bone_transform = local_rest_transform.affine_inverse()*bone_transform
	bone_transform =  bone_transform
	
	
	if bone_id == 33 or bone_id == 9:
		bone_transform = bone_transform
